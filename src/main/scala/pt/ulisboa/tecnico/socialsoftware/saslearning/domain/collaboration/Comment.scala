...
object Question {
  def fromUnsafe(content: String, author: User): Either[String, Question] = fromString(content)(Question(_, author))
}

object Answer {
  def fromUnsafe(content: String, author: User): Either[String, Answer] = fromString(content)(Answer(_, author))
}

object Definition {
  def fromUnsafe(content: String, author: User): Either[String, Definition] = fromString(content)(Definition(_, author))
}

object NeedMoreInformation {
  def fromUnsafe(content: String, author: User): Either[String, NeedMoreInformation] = fromString(content)(NeedMoreInformation(_, author))
}

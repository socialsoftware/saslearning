...
case class User(id: Option[Long] = None,
    ...
    extends Workable[User]
    with WithId {

  def createTeam(name: String,
                 owners: Set[User] = Set.empty,
                 members: Set[User] = Set.empty): Either[String, Team] =
    Team.fromUnsafe(name, Seq.empty, owners + this, members)

  override protected def updated(workspaces: Seq[Workspace]): User =
...
object User {
  ...
  def fromUnsafe(id: Option[Long] = None,
                 username: String,
                 email: String,
                 displayName: String): Either[String, User] =
    for {
  ...
  import pt.ulisboa.tecnico.socialsoftware.saslearning.utils.JsonUtils.{
    internetAddressDecoder,
    internetAddressEncoder
  }
  ...
  def fromJson(json: Json,
    ...
    (for {
      ...
      User
        .fromUnsafe(username = username, email = email, displayName = name)
        .toOption).toOption.flatten
  }
}

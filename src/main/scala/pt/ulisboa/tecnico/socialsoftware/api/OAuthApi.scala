...
import io.finch._
import pt.ulisboa.tecnico.socialsoftware.domain.Database
import pt.ulisboa.tecnico.socialsoftware.services.OAuthService
import shapeless.{:+:, CNil}

case class OAuthApi(oauthService: OAuthService) extends Api[Unit :+: String :+: CNil] {
  private val client = oauthService.client
  private val path = client.name.toLowerCase.trim

  def register: Endpoint[Unit] = get(path :: "register") {
    val destination =
  ...
  def callback: Endpoint[String] = get(path :: "callback" :: param("code")) { code: String =>
    (for {
